...
{
		setup_gpio_pins();
		StateQueue = xQueueGenericCreate( ( 20 ), ( sizeof(uint8_t) ), ( ( ( uint8_t ) 0U ) ) );
		MovReqQueue = xQueueGenericCreate( ( 20 ), ( sizeof(MovReq) ), ( ( ( uint8_t ) 0U ) ) );
		GPIOPinWrite(0x40025000, 0x00000004, 0);
		GPIOPinWrite(0x40025000, 0x00000008, 0);

		xTaskCreate(Task_WND_LS, "LimitSwitchesTask", 240, 0, 1, 0);
		xTaskCreate(Task_JAM_LS, "JamProtectionTask", 240, 0, 1, 0);
		xTaskCreate(Task_LOCK_BTN, "LockPBTask", 240, 0, 1, 0);
		xTaskCreate(Task_PASS_PB, "PassPB", 240, 0, 1, 0);
		xTaskCreate(Task_DRVR_PB, "DriverPB", 240, 0, 1, 0);
		xTaskCreate(Task_DRVR_AUTO, "DriverAUTOPB", 240, 0, 1, 0);
		xTaskCreate(TASK_ProcessInput, "ProcessingInputTask", 240, 0, 2, 0);
		xTaskCreate(TASK_MovReqExecution, "MovingTask", 240, 0, 3, 0);
